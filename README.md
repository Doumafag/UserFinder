# UserFinder
## Информация об авторе
**Имя и фамилия:** Кочеткова Виктория Вячеславовна
**Курс:** Итоговая аттестация по разработке GUI-приложений  
**Дата:** 28.04.2026
**Версия:** 1.0
## Описание
GUI-приложение для поиска пользователей GitHub с использованием официального GitHub API. Приложение позволяет:
1. Искать пользователей GitHub по имени
2. Просматривать детальную информацию о пользователе
3. Добавлять пользователей в избранное
4. Сохранять список избранных в JSON файл
5. Просматривать список избранных пользователей

## Технологии
1. Python 3.7+
2. Tkinter (GUI)
3. Requests (HTTP запросы)
4. JSON (хранение данных)

## Как использовать API

### GitHub REST API v3
Приложение использует следующие эндпоинты GitHub API:

#### 1. Поиск пользователей
GET https://api.github.com/search/users?q={query}&per_page=20

text
Параметры:
- `q` - поисковый запрос (логин пользователя)
- `per_page` - количество результатов на странице (макс. 100)

#### 2. Получение информации о пользователе
GET https://api.github.com/users/{username}

Установка и запуск
1. Клонирование репозитория
bash
git clone (https://github.com/Doumafag/UserFinder.git)
cd github-UserFinder
2. Установка зависимостей
bash
pip install -r requirements.txt
3. Запуск приложения
bash
python main.py



#### Пример 1: Поиск пользователя
В поле поиска введите "_" (любого известного вам пользователя)

Нажмите "Найти" или Enter

В списке появятся пользователи с логином, содержащим "_" (любого известного вам пользователя)

Кликните на пользователя для просмотра деталей

#### Пример 2: Добавление в избранное
Найдите пользователя 

Выберите его из списка

Нажмите "Добавить в избранное"

Пользователь сохранится в favorites.json

#### Пример 3: Просмотр избранного
Нажмите кнопку "Избранное"

Откроется окно со списком всех избранных пользователей

Видна дата добавления каждого пользователя
